...
description: Use CX Linux with Docker for containerized development and deployment. Build, manage, and orchestrate containers.
...
- Docker installed (or let CX Linux install it)
- CX Linux installed
...
## Docker and CX Linux Best Practices

### Using CX Linux Inside Containers

```dockerfile
# Dockerfile with CX Linux
FROM ubuntu:22.04

# Install CX Linux
RUN curl -fsSL https://cxlinux-ai.com/install.sh | bash

# Use CX Linux for setup
RUN cx "install python development environment"
```

### CX Linux with Docker Host

```bash
# Manage host docker from CX Linux
