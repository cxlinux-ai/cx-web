...
description: Configure and use different AI backends with CX Linux. Local models, OpenAI, Anthropic, and more.
keywords: [ai, backends, models, openai, anthropic, ollama, local]
---

# Supported AI Backends

CX Linux supports multiple AI backends for natural language processing. Choose based on your privacy requirements, performance needs, and internet connectivity.

## Backend Comparison

| Backend | Privacy | Speed | Offline | Cost |
|---------|---------|-------|---------|------|
| Local (CX) | Excellent | Fast | Yes | Free |
...
The built-in CX model runs entirely on your machine. No data leaves your system.
...
|-------|------|--------------|----------|
| `cx-3b` | 3B params | 4GB | Low-resource systems |
| `cx-7b` | 7B params | 8GB | General use (default) |
| `cx-13b` | 13B params | 16GB | Complex operations |

### Setup

The local model is installed with CX Linux:
...
cx backend download cx-13b

# Set default model
cx config set backend.model cx-13b
```
...
cx backend set ollama

# Specify model
cx config set backend.model codellama
...
cx backend config openai --api-key "sk-..."
...
cx backend set anthropic

# Set model
cx config set backend.model claude-3-sonnet
...
```bash
# Quick switch
cx --backend openai "complex analysis task"
cx --backend local "simple package install"
...
Create `.cx.yaml` in your project:

```yaml
# .cx.yaml - project-specific settings
backend:
  provider: openai
  model: gpt-4
```
...
CX_DEBUG_BACKEND=true cx "your command"
```
...
# Good: Secret manager
backend:
  api_key: ${vault:secret/cx/openai}
