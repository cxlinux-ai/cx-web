...
// ==========================================
// FOUNDING 1000 REFERRAL CODE GENERATION
// ==========================================

/**
 * Generate 12-character Founding 1000 referral code
 */
function generateFoundingReferralCode(): string {
  const chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"; // Removed confusing chars (0,O,1,I,L)
  let code = "";
  for (let i = 0; i < 12; i++) {
    code += chars.charAt(Math.floor(Math.random() * chars.length));
...
/**
 * POST /api/referral/founding-generate
 * Generate Founding 1000 member referral code with enhanced features
 */
router.post("/founding-generate", rateLimit({ windowMs: 1000 * 60, max: 5 }), async (req: Request, res: Response) => {
  try {
    const { referralCode, email, subscriptionId, planName } = req.body;
    const ip = (req.ip || req.connection.remoteAddress || '127.0.0.1') as string;

    // Use provided code or generate new one
    const finalCode = referralCode || generateFoundingReferralCode();

    // Check if code already exists (rare with 12 characters)
    const existingCode = await db
      .select()
      .from(ipReferralCodes)
      .where(eq(ipReferralCodes.referralCode, finalCode))
      .limit(1);

    if (existingCode.length > 0) {
      // Generate a new one if collision
      const newCode = generateFoundingReferralCode();
      await db.insert(ipReferralCodes).values({
        ipAddress: ip,
        referralCode: newCode,
        totalReferrals: 0,
        clickCount: 0,
        userAgent: req.get('User-Agent') || '',
        fingerprint: `founding-${Date.now()}`,
      });

      // Log founding member creation event
      await db.insert(referralEvents).values({
        referralCode: newCode,
        eventType: 'founding_member_created',
    ...
    console.error("Founding referral generation error:", error);
    res.status(500).json({ error: "Failed to generate founding referral code" });
  }
});
